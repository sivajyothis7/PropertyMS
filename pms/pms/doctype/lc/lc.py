...
def make_lease_payment_schedule(leasedoc):
    ...
    for lease_item in lc.lease_item:
        ...
        if lease_item.frequency == "Daily":
            invoice_qty = 1.00
            frequency_calculation_function = add_days
        else:
        ...
        while rent_end_date >= rent_start_date:
            if lease_item.frequency == "Daily":
                invoice_period_end = frequency_calculation_function(rent_start_date, frequency_factor - 1)
            else:
            ...
            if invoice_period_end > rent_end_date:
                if lease_item.frequency == "Daily":
                    ...
                    invoice_qty = getDateMonthDiff(rent_start_date, rent_end_date, frequency_factor)
            ...
            if lease_item.frequency == "Daily":
                rent_start_date = add_days(rent_start_date, frequency_factor)
            else:
